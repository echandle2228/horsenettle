...
CCHPL_lmer1 <- lmer(CHPL ~ location + (1|ID), data = C.CHPL)

Anova(CCHPL_lmer1, type="III")

postHocs <- glht(CCHPL_lmer1, linfct = mcp(location = "Tukey"))
summary(postHocs)
...
reg
indiv <- bartlett.test(CHPL ~ ID, data = C.CHPL)
indiv
```
...
# Saving graph
ggsave("figures/C.CHPL.POPULATION.png", width = 7, height = 5, dpi = 300)
