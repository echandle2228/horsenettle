...
PS_CPS <- PS[!is.na(PS$CPS),]

PS1 <- PS %>% 
  group_by(location) %>% 
  filter(!CPS <= (mean(PS_CPS$CPS) - SE(PS_CPS$CPS)))
...
# Cold Treatment
hist(PS$CPS, main = "Cold Treatment", xlab = "Photosynthesis")
shapiro.test(PS$CPS)

# Hot Treatment
hist(PS$HPS, main = "Hot Treatment", xlab = "Photosynthesis")
shapiro.test(PS$HPS)
...
# Model
CPS_lmer1 <- lmer(CPS ~ location + (1|ramet) + (1|population:ID), data = PS)
CPS_lmer1 <- lmer(CPS ~ location + (1|ramet) + (1|population), data = PS)
CPS_lmer1 <- lmer(CPS ~ location + (1|ramet), data = PS)

CPS_aov <- aov(CPS ~ location, data = PS)
summary(CPS_aov)


# Data Visualization
(CPS.p1 <- ggplot(PS, aes(x = location, CPS, fill = location)) + 
...
# Model
HPS_lmer1 <- lmer(HPS ~ location + (1|ramet) + (1|population:ID), data = PS)
HPS_lmer1 <- lmer(HPS ~ location + (1|ramet) + (1|population), data = PS)
...
# Data Visualization
(HPS.p1 <- ggplot(PS, aes(x = location, HPS, fill = location)) + 
    geom_boxplot(alpha = 0.8) +
    scale_fill_manual(values = c("#6CA6CD", "#F08080")) +
    HStheme() +
    theme(legend.position = "none") +
