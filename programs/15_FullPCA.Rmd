---
title: "Sporophytic Tolerance PCA"
author: "Emma Chandler"
date: "September 12, 2021"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

## Load libraries
```{r message=FALSE}

library(rgl)
library(dplyr)
library(ggfortify)
library(pca3d)
library(factoextra)

library(GGally)

```

## Load data
```{r}
remove(list = ls())

data <- read.csv("~/Horsenettle/data/processed/correlationFull.csv", header = TRUE)


```

# Creating a figure theme for all plots
```{r message=FALSE}
HStheme <- function(){            
  theme_bw()+                          
  theme(axis.text.x = element_text(size = 12), 
        text = element_text(family = "sans"), 
        axis.text.y = element_text(size = 12),
        plot.subtitle = element_text(size = 16, vjust = 1, hjust = 0.5),
        axis.title = element_text(size = 14),
        panel.grid = element_blank(),
        plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), units = , "cm"),
        plot.title = element_text(size = 20, vjust = 1, hjust = 0.5),
        legend.text = element_text(size = 12),
        legend.title = element_blank())
}
```


## PCA of all variables except photosynthesis
```{r}

# Drop photosynthesis
data.noPS <- data %>% 
  dplyr::select(,1:14) %>% 
  na.omit()


# Omitt all rows with nas
data.nona <- na.omit(data)


ALL.PCA <- prcomp(data.noPS[,c(7:14)], center = TRUE, scale. = TRUE)

summary(ALL.PCA)
print(ALL.PCA)


(PCA.p1 <- autoplot(prcomp(data.noPS[,c(7:14)], center = TRUE, scale. = TRUE), 
         data = data.noPS, colour = 'location', 
         loadings = TRUE, 
         loadings.colour = 'black',
         loadings.label = TRUE, 
         loadings.label.size = 4, 
         loadings.label.colour = 'black', 
         loadings.label.hjust = 1, 
         loadings.label.vjust = 0,
         frame = TRUE, frame.type = 't') +
  scale_fill_manual(values = c("#104E8B", "#B22222")) +
  scale_color_manual(values = c("#104E8B", "#B22222")) +
  theme(plot.margin = unit(c(0.5,0.5,-2,0.5), "cm"))+
  xlim(-0.5,0.5) +
  ylim(-0.5,0.5) +
  HStheme()) 

# Saving graph
# ggsave("figures/PCA_full_1.png", width = 7, height = 5, dpi = 300)

(PCA.p2 <- autoplot(prcomp(data.noPS[,c(7:14)], center = TRUE, scale. = TRUE), x=2, y=3, 
         data = data.noPS, colour = 'location', 
         loadings = TRUE, 
         loadings.colour = 'black',
         loadings.label = TRUE, 
         loadings.label.size = 4, 
         loadings.label.colour = 'black', 
         loadings.label.hjust = 0, 
         loadings.label.vjust = 0,
         frame = TRUE, frame.type = 't') +
  scale_fill_manual(values = c("#104E8B", "#B22222")) +
  scale_color_manual(values = c("#104E8B", "#B22222")) +
  xlim(-0.5,0.5) +
  ylim(-0.5,0.5) +
  HStheme())

# Saving graph
# ggsave("figures/PCA_full_2.png", width = 7, height = 5, dpi = 300)

(PCA.p3 <- autoplot(prcomp(data.noPS[,c(7:14)], center = TRUE, scale. = TRUE), x=1, y=3, 
         data = data.noPS, 
         colour = 'location', 
         loadings = TRUE, 
         loadings.colour = 'black',
         loadings.label = TRUE, 
         loadings.label.size = 4, 
         loadings.label.colour = 'black', 
         loadings.label.hjust = 0, 
         loadings.label.vjust = 0,
         frame = TRUE, frame.type = 't') +
  scale_fill_manual(values = c("#104E8B", "#B22222")) +
  scale_color_manual(values = c("#104E8B", "#B22222")) +
  theme(plot.margin = unit(c(-2,0.5,0.5,0.5), "cm"))+
  xlim(-0.5,0.5) +
  ylim(-0.5,0.5) +
  HStheme())

# Saving graph
# ggsave("figures/PCA_full_3.png", width = 7, height = 5, dpi = 300)

(stacked_PCA <- ggpubr::ggarrange(PCA.p1, PCA.p2, PCA.p3, ncol = 1,
                                  labels = c("A", "B", "C"),
                                  common.legend = TRUE, 
                                  legend = "right"))


# Saving graph
ggsave("figures/PCA_full_stacked.png", width = 6, height = 10, dpi = 300)


# https://www.datacamp.com/community/tutorials/pca-analysis-r
```


## 3D plot with first 3 principal componenets
```{r message = FALSE}
SPOR.PCA <- prcomp(data.noPS[,c(7:14)], center = TRUE, scale. = TRUE)

scores = as.data.frame(SPOR.PCA$x)
summary(SPOR.PCA)

plot3d(scores[,1:3], col=c(1:4), size=10, type='p', 
       xlim = c(-50,50), ylim=c(-50,50), zlim=c(-50,50))
text3d(scores[,1]+2, scores[,2]+10, scores[,3]+2,
       texts=c(rownames(scores)), cex= 0.7, pos=3)

loca <- factor(data.noPS$location)


pca3d(SPOR.PCA, group = loca,
      show.ellipses=TRUE,
      ellipse.ci=0.75,
      show.plane=FALSE,
      palette = c("#00BFFF", "#FF4040"),)


dir.create("animation_merge")

for (i in 1:360) {
  view3d(userMatrix=rotationMatrix(2*pi * i/360, 0, 1, 0))
  rgl.snapshot(filename=paste("animation_merge/frame-",
                              sprintf("%03d", i), ".png", sep=""))
  }


movie3d(movie = "3dAnimatedScatterplot",
        spin3d(axis = c(0,0,1), rpm = 7),
        duration = 10,
        type = "gif",
        dir = "~/Horsenettle/programs/animation_merge",
        clean = TRUE)


# snapshotPCA3d(file="first_plot.png")
```


Get PCA individual data
```{r}
ID_PCA <- get_pca_ind(ALL.PCA)
head(ID_PCA$coord)



coor <- predict(ALL.PCA, newdata = data.noPS)

ALL <- cbind(data.noPS, coor)


t.test(PC1 ~ location, ALL)
t.test(PC2 ~ location, ALL)
t.test(PC3 ~ location, ALL)

PC2.m <- aov(PC2 ~ location, data = ALL)
summary(PC2.m)

# Data Visualization
(PC1 <- ggplot(ALL, aes(x = location, PC2, fill = location)) +     
    geom_boxplot(alpha = 0.8) +
    scale_fill_manual(values = c("#104E8B", "#B22222")) +
    labs(x = "Region", y = "PC2")+
    HStheme() +
    theme(legend.position = "none"))

# Saving graph
# ggsave("figures/SPOR_PC1.png", width = 7, height = 5, dpi = 300)

```









