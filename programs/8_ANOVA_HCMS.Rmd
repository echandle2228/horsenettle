...
# Data Visualization
(H.CMS.p1 <- ggplot(H.CMS, aes(x = reorder(ramet, CMS), CMS, fill = location)) + 
...
CMS.SUMM = H.CMS %>% 
  group_by(ID) %>% 
  summarize(maxCMS=max(CMS))

(hsd=HSD.test(HCMS_aov2,"Genotype", group=T))

ggplot(H.CMS,aes(x=reorder(Species,Petal.Length,FUN = median),y=Petal.Length))+
  geom_boxplot()+
  geom_text(data=CMS.SUMM,aes(x=Species,y=0.2+Max.Petal.Length,label=hsd$groups$groups),vjust=0)


# I need to group the treatments that are not different each other together.
labeldf <- function(TUKEY, variable){
...
ID <- as.character(H.CMS$ID) 
# Apply the function on my dataset
LABELS <- labeldf(TUKEY , "ID")


#Crate summarised data
detach(package:plyr)

summary <- H.CMS %>% group_by(ID) %>% 
