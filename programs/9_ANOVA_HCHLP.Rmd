...
HCHPL_lmer1 <- lmer(CHPL ~ location + (1|ID), data = H.CHPL)

Anova(HCHPL_lmer1, type="III")

postHocs <- glht(HCHPL_lmer1, linfct = mcp(location = "Tukey"))
summary(postHocs)
...
```{r}
reg <- levene.test(H.CHPL$CHPL, H.CHPL$location)
reg

reg <- bartlett.test(CHPL ~ location, data = H.CHPL)
reg

indiv <- bartlett.test(CHPL ~ ID, data = H.CHPL)
indiv
```
...
# Saving graph
ggsave("figures/H.CHPL.POPULATION.png", width = 7, height = 5, dpi = 300)
