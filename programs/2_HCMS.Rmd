...
HCMSL.origin <- lmer(exp_max ~ location + (1|ramet) + (1|ID), data = H.CMS)

summary(HCMSL.origin)
anova(HCMSL.origin)
...
```{r}
HCMS_lmer1 <- lmer(CMS ~ location + (1|ramet) + (1|ID), data = H.CMS)
AIC(HCMS_lmer1)
(HCMS_lmer1.a <- anova(HCMS_lmer1))
(HCMS_glmer1.r <- ranova(HCMS_lmer1))

HCMS_lmer1_pop <- lmer(CMS ~ location + population + (1|ramet) + (1|ID), data = H.CMS)
AIC(HCMS_lmer1_pop)
(HCMS_lmer1_pop.a <- anova(HCMS_lmer1_pop))
(HCMS_lmer1_pop.r <- ranova(HCMS_lmer1_pop))


summary(HCMS_lmer1)

# Include populations as a random effect with southern populations pooled
H.CMS <- H.CMS %>% mutate(population_2 = case_when(location == "South" ~ "South",
                                                   population == "Prairie Island" ~ "Prairie Island",
                                                   population == "Frontenac" ~ "Frontenac"))

HCMS_lmer1_pop <- lmer(CMS ~ location + (1|ramet) + (1|population_2/ID), data = H.CMS)
AIC(HCMS_lmer1_pop)

(HCMS_lmer1_pop.a <- anova(HCMS_lmer1_pop))
(HCMS_lmer1_pop.r <- ranova(HCMS_lmer1_pop))

summary(HCMS_lmer1_pop)
...
(CMS.block.plot <- ggplot(H.CMS, aes(x = ramet, y = CMS, fill = location)) + 
    geom_boxplot(alpha = 0.8) +
    scale_fill_manual(values = c("#104E8B", "#CD2626")) +
    HStheme() +
...
# Saving graph
# ggsave("figures/H.CMS.BLOCK.png", width = 7, height = 5, dpi = 300)
...
(H.CMS.dot <- ggplot(H.CMS, aes(x = reorder(ID, CMS, FUN = median, na.rm = TRUE), y = CMS, fill = location)) +
    geom_boxplot(alpha = 0.8) +
    HStheme() +
    scale_fill_manual(values = c("#104E8B", "#B22222"))+
    theme(axis.text.x = element_text(angle = 90, size = 8, vjust = 0.5),
          axis.text.y = element_text(size = 8))+
    labs(x = "Genet", y = "CMS Ratio", title = "Cell Membrane Stability", subtitle = "Hot Treatment"))
