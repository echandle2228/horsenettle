...
ggplot(germ_dat, aes(x = temp, y = germ_percent, color = location)) +
  ...
  theme(text = element_text(family = "sans"), 
        ...
        panel.grid = element_blank(),
        plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), units = , "cm"),
        legend.text = element_blank(size = 12),
        legend.title = element_blank())
...
## Linear mixed effects models for Tmax, Topt, and Tmin
```{r}
# Include populations as a random effect with southern populations pooled
bilin <- bilin %>% mutate(population_2 = case_when(location == "South" ~ "South",
...
PGERM_max.R <- lmer(ctmax ~ location + population + (1|ID), data = bilin)
...
PGERM_opt.R <- lmer(topt ~ location + population + (1|ID), data = bilin)
...
PGERM_min.ID_noOut <- lmer(ctmin ~ location + population + (1|ID), data = bilin_noOut)

anova(PGERM_min.ID_noOut)
ranova(PGERM_min.ID_noOut)
...
## Summary and tmax figure
```{r}

# Is there a difference between Regions in Tmax
PGERM_max.R <- lmer(ctmax ~ location + (1|ID), data = bilin)
summary(PGERM_max.R)
...
(raw_dat <- ggplot(germ_dat, aes(x = temp, y = germ_percent, color = location)) +
...
(dot_plot <- ggerrorplot(bilin, x = "location", y = "ctmax", desc_stat = "mean_se", color = "location") +
  theme_classic() +
  labs(x = "Region", y = "Tmax °C")+
  scale_colour_manual(values = c("#B22222", "#104E8B")) +
  theme(text = element_text(family = "sans"),
        axis.text.x = element_text(size = 12),
        axis.text.y = element_text(size = 12),
        axis.title = element_text(size = 12),
...
(Pub_GermFig <- ggarrange(raw_dat, dot_plot,
                    labels = c("A", "B"),
                    ncol = 2, nrow = 1,
                    widths = c(1.25, 0.75),
                    common.legend = TRUE,
...
ggsave("figures/Pub_GermFig.png", width = 7, height = 3.5, dpi = 500)
